...
#// threading
def chkforinputThread():
	threading.Thread(target=chkforinput).start()
def downloadMediaThreading():
	threading.Thread(target=downloadMedia).start()
	threading.Thread(target=progresBarFxn).start()
	# lambda:[downloadMedia(), progresBarFxn()]
#// threading
...
downButton = Button(root, text="", bg="#5e5e5a", fg="White",font=("Helvetica",12),command=downloadMediaThreading,state="normal")
####PROGRESS baR
...
Label(root,text=" ",bg="#5e5e5a").pack()
b1=Button(root, text="   Verify   ", bg="Black", fg="White",font=("Helvetica",12),command=chkforinputThread)
b1.pack()
